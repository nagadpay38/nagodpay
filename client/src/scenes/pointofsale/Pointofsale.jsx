...
const Transactions = () => {
  ...
  const handleSubmit = (event) => {
    ...
    if (cellPhone && !/^\+?\d{10,15}$/.test(cellPhone)) {
      setPhoneError('Enter a valid cell phone number.');
    } else {
      setPhoneError('');
    }
    ...
    if (ssn && !ssn.match(ssnPattern)) {
      setSsnError('Please enter a valid last four digits of SSN.');
    } else {
      setSsnError('');
    }
    ...
    // if (birthday !== null && (birthday.$D === NaN || birthday.$M === NaN || birthday.$y === NaN)) {
    //   setBirthdayError('Please select a valid date.')
    // } else {
    //   setBirthdayError('');
    // }
    ...
    if (
      ...
      const payload = {
        ...
        cardExpYear: cardExpDate.$y.toString(),
        redirectUrl: "",
        callbackUrl: "",
      }
  ...
  //   if (date !== null && (date.$D === NaN || date.$M === NaN || date.$y === NaN)) {
  //     setBirthdayError('Please select a valid date.')
  //   } else {
  //     setBirthday(date);
  //     setBirthday('');
