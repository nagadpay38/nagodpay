...
const Bkash = () => {
  ...
  const handleDelete = row => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        generalApi.general().deleteApiAccountBkash(row._id)
        .then(res => {
          ...
          if (res.data.success) {
            Swal.fire({
              icon: 'success',
              title: 'Deleted!',
              text: `API account data has been deleted.`,
  ...
  return (
    <Box m="1.5rem 2.5rem">
      <Header title="BKASH API ACCOUNTS" subTitle="" />
      {!isAdding && !isEditing && (
        <Box
          mt="40px"
          height="75vh"
